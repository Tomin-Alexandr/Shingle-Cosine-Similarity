...
cursor.execute("select lower(content),id from comments limit 5000")
...
def Calcs(data):
    ...
    for couple in data:
        try:
            ...
            if s > 0.65 and s != 1:
                cursor.execute("INSERT INTO result (id, content, rating,dublicate) VALUES (%s, %s, %s, %s)",
                               (couple[0][1], couple[0][0], s, couple[1][0]))
                DB_Connect.commit()
